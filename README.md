...
The algorithm definitions, test vectors (JWKs and JWEs), RFC 8792-folded
presentation copies of the test vectors, and several draft sections
(algorithm tables, IANA registrations, test vectors appendix) are generated
by scripts in the `scripts/` folder. The algorithm suites are defined in
...
When the algorithm definitions have changed, or when `--force` is used,
this will:
...
1. Generate RFC 8792-folded presentation copies under `examples/folded/`
   for inclusion in the draft
2. Update the draft markdown with regenerated algorithm identifier tables,
