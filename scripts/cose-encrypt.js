...
const coseKeysDir = join(examplesDir, "cose-keys");
const outDir = join(examplesDir, "cose");
...
for (const { alg, kem, kdf, aead, coseValue } of algorithms.filter(a => a.cose)) {
  ...
  if (isKE) {
    ...
    let contentAlg, contentCipher, cekSize;
    switch (suite.AEAD.id) {
      case 0x0001: // AES-128-GCM
        contentAlg = CONTENT_ALG_A128GCM;
        contentCipher = "aes-128-gcm";
        cekSize = 16;
        break;
      case 0x0002: // AES-256-GCM
        contentAlg = CONTENT_ALG_A256GCM;
        contentCipher = "aes-256-gcm";
        cekSize = 32;
        break;
      case 0x0003: // ChaCha20Poly1305
        contentAlg = CONTENT_ALG_CHACHA20POLY1305;
        contentCipher = "chacha20-poly1305";
        cekSize = 32;
        break;
      default:
    ...
    const cek = new Uint8Array(randomBytes(cekSize));
    ...
    vectors.push({ alg, key: keyHex, hex });
  } else {
...
writeFileSync(
  join(examplesDir, "cose-vectors.json"),
